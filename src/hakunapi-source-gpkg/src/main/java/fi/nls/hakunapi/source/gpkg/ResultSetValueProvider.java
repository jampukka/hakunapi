...
public class ResultSetValueProvider implements ValueProvider {
    ...
    /** Zero-copy blob route, or null when the driver does not expose it. */
    private final SqliteBlobReader blobReader;

    public ResultSetValueProvider(ResultSet rs, int numCols) {
        this(rs, numCols, null);
    }

    public ResultSetValueProvider(ResultSet rs, int numCols, SqliteBlobReader blobReader) {
        this.rs = rs;
    ...
    /**
     * The row's geometry. With a {@link SqliteBlobReader} the bytes are read
     * through a buffer over SQLite's own memory rather than copied into a fresh
     * {@code byte[]} per feature - so the returned geometry, like the provider
     * itself, is only valid until the cursor advances.
     */
    @Override
    public HakunaGeometry getHakunaGeometry(int i) {
        try {
            if (blobReader != null) {
                ByteBuffer bb = blobReader.read(i);
                return bb == null ? null : new HakunaGeometryGPKG(bb);
            }
